...
def fdct_wrapping_window(x):
        wr = numpy.zeros(x.shape)
        wl = numpy.zeros(x.shape)
        x[numpy.abs(x) < 2**(-52)] = 0 #Why?
        wr[(x>0) * (x<1)] = numpy.exp((1 - 1 / (1 - numpy.exp((1- 1 / x[(x>0) * (x<1)])))))
        wr[x <= 0] = 1
        wl[(x>0) * (x<1)] = numpy.exp((1 - 1 / (1 - numpy.exp((1- 1 / (1 - x[(x>0) * (x<1)]))))))
        wl[ x >= 1] = 1
        normalization = numpy.sqrt(wl**2+wr**2)
        wr = wr / normalization
        wl = wl / normalization
        return wl,wr