...
class RegionGrow():
	...
	def getRegionGrowImage(self,I,position,thres=60):
		'''
		Generate region grow image,default threshold is 60
		when threshold is small,the image is sharp and has less white
		when threshold is big,most region will be reserved.
		'''
		return self._region_gorw(I,position,thres)
		
	def boundex(self,I,threshold):
		(m,n)=I.shape
		result = np.zeros((m+2,n+2))
		result[1:m+1,1:n+1]=I.copy()
		
		result[1:m,0] =2 * threshold #* np.ones((m-1,1)) #+result[1:m,0]
		result[1:m,n+1]=2 * threshold #* np.ones((m-1,1)) #+ result[1:m,m+1]
		result[0,1:n] = 2 * threshold #* np.ones((1,n-1)) #+ result[0,1:n] 
		result[m+1,1:n] = 2 * threshold #* np.ones((1,n-1)) #+ result[m+1,1:n]
		return result
		
	def _region_gorw(self,I,position,thres=60):
		'''
		Region Growing Algorithm
		I : input image
		position: selected seed position
		thres:threshold of this algorithm
		'''
		assert len(I.shape) == 2
		Itermax=500000
		threshold=thres#%输入阈值，1~255之间的整数
		flag=1#%have taken the pixel?
		I=self.nor1(I)
		boundexI = self.boundex(I,thres)
		#I = norshow(boundexI)
...
if __name__ == "__main__":
	...
	rg = RegionGrow()
	img = rg.getRegionGrowImage(img,SELECTEDPOINT,60)
	showImg(img)